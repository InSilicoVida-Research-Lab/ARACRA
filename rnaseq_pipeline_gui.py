    ...
    try:
        ...
        with tab1:
            ...
            with st.sidebar:
                ...
                default_base = str(Path.home() / "rnaseq_pipeline")
                base_output_dir = st.text_input("Base Output Directory", value=default_base)
                trimmomatic_jar = st.text_input("Trimmomatic JAR Path", value="/path/to/trimmomatic.jar")
                adapters = st.text_input("Adapter Sequences", value="/path/to/adapters.fa")
                ...
                max_cpu = min(os.cpu_count() or 4, 16)
                num_processes = st.slider("Parallel processes", 1, max_cpu, min(4, max_cpu))
