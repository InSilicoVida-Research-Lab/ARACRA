...
# Activate rnaseq_pipeline env
ENV_NAME="rnaseq_pipeline2"
if conda activate "$ENV_NAME" 2>/dev/null; then
    echo "  ✔ Activated: ${ENV_NAME}"
else
    echo "ERROR: Environment '${ENV_NAME}' not found."
    echo "  Create it:  mamba env create -f environment.yml"
    exit 1
fi
...
echo ""
echo "  Tool check:"
for tool in nextflow STAR samtools fastp featureCounts salmon multiqc streamlit; do
